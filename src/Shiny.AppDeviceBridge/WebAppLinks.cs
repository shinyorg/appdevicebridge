...
public sealed class WebAppLinks(WebAppLinkOptions options)
{
    ...
    public event Action<AppLink>? Received;
    ...
    public bool Receive(Uri uri)
    {
        ...
        var link = new AppLink(uri.OriginalString, route, DateTimeOffset.UtcNow);
    ...
    public static bool IsLocalRoute(string? route)
    {
        ...
        // The default mount points. A host that moved them still accepts these as routes, which is harmless:
        // the redirect that consumes a link is checked against the server's own paths when it runs.
        return !IsUnder(route, WebAppPaths.DefaultBridgePrefix) && !IsUnder(route, WebAppPaths.HostSegment);
    }
...
public sealed class WebAppLinksBridge : IWebAppBridge, IDisposable
{
    ...
    static ValueTask Respond(HttpContext context, AppLink? link) => link is null
