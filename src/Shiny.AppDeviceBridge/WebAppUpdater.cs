...
public sealed class WebAppUpdater : IDisposable
{
    ...
    internal WebAppUpdater(WebAppHostOptions options, WebAppInstallStore store, ILogger logger)
    {
    ...
    string? Verify(WebAppUpdateResponse response, WebAppVersion? current, Uri checkUri)
    {
        ...
        if (!String.Equals(release.AppId, this.options.AppId, StringComparison.Ordinal))
        ...
        if (release.MinimumHostVersion is { } minimum
            && (!WebAppVersion.TryParse(minimum, out var minimumHost) || this.options.ParsedHostVersion < minimumHost))
