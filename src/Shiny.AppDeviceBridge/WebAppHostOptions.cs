...
public sealed class WebAppHostOptions
{
    ...
    /// endpoints an older app does not have. <c>Shiny.AppDeviceBridge.Maui</c> fills this in from the
    ...
    /// <summary>
    /// The path everything is served under — pages, assets and bridges alike. <c>/</c> by default.
    /// <para>
    /// The web app has to agree: its <c>&lt;base href&gt;</c> must be this path. The host rewrites the entry
    /// document's <c>&lt;base href&gt;</c> as it serves it, so a Blazor publish with the usual
    /// <c>&lt;base href="/" /&gt;</c> works unchanged, but anything the app builds by hand from an absolute
    /// <c>/</c> will not.
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// Where the bridges mount, under <see cref="BasePath"/>. <c>/_bridge</c> by default; move it when the web
    /// app wants that route for itself. The page discovers it from <c>{base}/_host/config</c> rather than
    /// being configured to match, so the two halves cannot drift apart when they ship separately.
    /// </summary>
    ...
    /// <summary>Where installed builds and the <c>data</c> root live: <see cref="InstallDirectory"/>, or a folder under local app data.</summary>
    ...
    internal void Validate()
    {
        ...
        if (!WebAppPaths.IsValid(this.BasePath))
            throw new InvalidOperationException($"WebAppHostOptions.BasePath '{this.BasePath}' is not valid. Use path segments of letters, digits, '-', '_', '.' and '~'.");

        if (!WebAppPaths.IsValid(this.BridgePrefix))
            throw new InvalidOperationException($"WebAppHostOptions.BridgePrefix '{this.BridgePrefix}' is not valid. Use path segments of letters, digits, '-', '_', '.' and '~'.");

        var bridgePrefix = WebAppPaths.Normalize(this.BridgePrefix);

        if (bridgePrefix.Length == 0)
            throw new InvalidOperationException("WebAppHostOptions.BridgePrefix cannot be the root: the bridges need a prefix of their own to sit under.");

        // _host is where a page looks to find everything else, so nothing may be mounted over it.
        if (bridgePrefix.Equals(WebAppPaths.HostSegment, StringComparison.OrdinalIgnoreCase)
            ...
            throw new InvalidOperationException($"WebAppHostOptions.BridgePrefix cannot be '{WebAppPaths.HostSegment}' or sit under it — that is where the session and the mount points themselves are served.");
