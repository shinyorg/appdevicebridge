...
namespace Shiny.WebAppHost.Bridge.Jobs;
...
/// <summary>The payload a <c>job:{name}</c> handler receives.</summary>
public sealed record WebAppJobPayload(string Name);
...
abstract class WebAppJobBase(WebAppInvoker invoker, WebAppJobCatalog catalog, ILogger logger) : IJob
{
    public async Task Run(CancellationToken cancelToken)
    {
        ...
        foreach (var name in catalog.NamesFor(this.GetType()))
        {
            ...
            var result = await invoker.InvokeAsync($"job:{name}", new WebAppJobPayload(name), WebAppJobJsonContext.Default.WebAppJobPayload, cancelToken);
...
[JsonSerializable(typeof(WebAppJobPayload))]
partial class WebAppJobJsonContext : JsonSerializerContext;
