...
public class WebAppNotificationDelegate(WebAppEventHub events, WebAppInvoker invoker, WebAppNotificationOptions options) : INotificationDelegate
{
    ...
    async Task HandleAsync(string name, Notification notification, string? action, string? text)
    {
        ...
        var payload = NotificationPayload.From(notification, action, text);

        // As with push: the event is for pages that only display; the handler call is the one that does work.
        events.Publish(name, payload, NotificationsBridgeJsonContext.Default.NotificationPayload);
        await invoker.InvokeAsync(name, payload, NotificationsBridgeJsonContext.Default.NotificationPayload);
    }
}
...
static class NotificationPlatform
{
    ...
    public static bool TryAttachImage(Notification notification, string path, out string? copy, out string? error)
    {
        ...
        var directory = Path.Combine(Path.GetTempPath(), "webapphost-notifications");
