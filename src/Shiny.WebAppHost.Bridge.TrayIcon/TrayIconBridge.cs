...
public sealed partial class TrayIconBridge : IWebAppBridge, IDisposable
{
    ...
    readonly WebAppHostOptions? hostOptions;
    ...
    public TrayIconBridge(IServiceProvider services, WebAppEventHub events)
    {
        ...
        this.hostOptions = services.GetOptionalService<WebAppHostOptions>();
    ...
    async ValueTask StatusAsync(HttpContext context)
    {
        ...
        try
        {
            await WebAppBridgeResults.Json(
                context,
                new TrayStatusResponse(this.IsSupported, this.options.MaxIcons, [.. this.icons.Values.Select(Describe)]),
                TrayBridgeJsonContext.Default.TrayStatusResponse
            );
        }
        finally
        {
    ...
    {
        var body = await WebAppBridgeResults.ReadBodyAsync(context, TrayBridgeJsonContext.Default.TrayIconRequest) ?? new TrayIconRequest();
    ...
    ValueTask GetAsync(HttpContext context)
        => this.WithEntry(context, entry => WebAppBridgeResults.Json(context, Describe(entry), TrayBridgeJsonContext.Default.TrayIconResponse));
    ...
    {
        var body = await WebAppBridgeResults.ReadBodyAsync(context, TrayBridgeJsonContext.Default.TrayMenuRequest);
        if (body is null)
        {
        ...
        await this.WithEntry(context, async entry =>
        {
            ...
            entry.Nodes = nodes;
            await WebAppBridgeResults.Json(context, Describe(entry), TrayBridgeJsonContext.Default.TrayIconResponse);
        });
    }
    ...
    {
        var body = await WebAppBridgeResults.ReadBodyAsync(context, TrayBridgeJsonContext.Default.TrayNotificationRequest);
    ...
    {
        var body = await WebAppBridgeResults.ReadBodyAsync(context, TrayBridgeJsonContext.Default.TrayAnimationRequest);
    ...
    async ValueTask UpsertAsync(HttpContext context, string? requestedId, TrayIconRequest body)
    {
        if (!this.EnsureSupported(context, out var notSupported))
        {
            await notSupported;
        ...
        try
        {
            ...
            await WebAppBridgeResults.Json(
                context,
                Describe(entry),
                TrayBridgeJsonContext.Default.TrayIconResponse,
    ...
    void Clicked(string id, TrayClickButton button, TrayClickEventArgs args)
        => this.Raise("tray.click", new TrayClickPayload(id, button, args.X, args.Y), TrayBridgeJsonContext.Default.TrayClickPayload);

    void MenuActivated(string id, string itemId, string? label, bool? isChecked)
        => this.Raise("tray.menu", new TrayMenuPayload(id, itemId, label, isChecked), TrayBridgeJsonContext.Default.TrayMenuPayload);
    ...
    static TrayIconResponse Describe(TrayEntry entry) => new(
        entry.Id,
        entry.Icon.Tooltip,
        entry.Icon.Title,
    ...
    static IReadOnlyList<TrayMenuItemResponse> Describe(List<TrayMenuNode> nodes) =>
    [
        .. nodes.Select(node => new TrayMenuItemResponse(
            node.Id,
            node.Type,
            node.Item switch
            {
