...
public sealed partial class TrayIconBridge
{
    ...
    bool TryBuildItems(
        string trayId,
        List<TrayMenuItemRequest> requests,
    ...
    static int Count(List<TrayMenuItemRequest> items)
    ...
    bool TryResolveImage(TrayImage? image, out Func<Stream>? factory, [NotNullWhen(false)] out string? error)
    {
        ...
        if (this.hostOptions is null || !WebAppFileRoots.TryResolve(this.hostOptions, image.Root, image.Path, out var fullPath))
        {
