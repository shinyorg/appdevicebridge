...
public sealed class GpsBridge : IWebAppBridge, IDisposable
{
    ...
    ValueTask StatusAsync(HttpContext context)
    {
        ...
        return WebAppBridgeResults.Json(context, new AccessResponse(g.GetCurrentStatus(mode)), LocationBridgeJsonContext.Default.AccessResponse);
    }
    ...
    async ValueTask RequestAccessAsync(HttpContext context)
    {
        ...
        var body = await WebAppBridgeResults.ReadBodyAsync(context, LocationBridgeJsonContext.Default.GpsListenerRequest) ?? new GpsListenerRequest();
        var access = await g.RequestAccess(body.ToRequest());

        await WebAppBridgeResults.Json(context, new AccessResponse(access), LocationBridgeJsonContext.Default.AccessResponse);
    }
    ...
    ValueTask ListenerAsync(HttpContext context)
    {
        ...
        return g.CurrentListener is { } listener
            ? WebAppBridgeResults.Json(context, GpsListenerRequest.From(listener), LocationBridgeJsonContext.Default.GpsListenerRequest)
    ...
    async ValueTask StartListenerAsync(HttpContext context)
    {
        ...
        try
        {
            await g.StartListener(body.ToRequest());
        }
    ...
    static ValueTask Reading(HttpContext context, GpsReading? reading)
        => reading is null
            ? WebAppBridgeResults.NoContent(context)
            : WebAppBridgeResults.Json(context, GpsReadingResponse.From(reading), LocationBridgeJsonContext.Default.GpsReadingResponse);

    void OnReading(object? sender, GpsReading reading)
    {
        if (this.events.HasSubscribers)
            this.events.Publish("gps.reading", GpsReadingResponse.From(reading), LocationBridgeJsonContext.Default.GpsReadingResponse);
    }
...
public sealed class GeofenceBridge(IServiceProvider services) : IWebAppBridge
{
    ...
    ValueTask StatusAsync(HttpContext context)
        => this.geofences is { } g
            ? WebAppBridgeResults.Json(context, new AccessResponse(g.CurrentStatus), LocationBridgeJsonContext.Default.AccessResponse)
    ...
    async ValueTask RequestAccessAsync(HttpContext context)
    {
        ...
        await WebAppBridgeResults.Json(context, new AccessResponse(await g.RequestAccess()), LocationBridgeJsonContext.Default.AccessResponse);
    }
    ...
    ValueTask ListAsync(HttpContext context)
    {
        ...
        List<GeofenceRegionRequest> regions = [.. g.GetMonitorRegions().Select(GeofenceRegionRequest.From)];
        return WebAppBridgeResults.Json(context, regions, LocationBridgeJsonContext.Default.ListGeofenceRegionRequest);
    }
    ...
    async ValueTask StartMonitoringAsync(HttpContext context)
    {
        ...
        var body = await WebAppBridgeResults.ReadBodyAsync(context, LocationBridgeJsonContext.Default.GeofenceRegionRequest);
        if (body is null)
        {
        ...
        if (!body.TryToRegion(out var region, out var error))
        {
    ...
    async ValueTask StateAsync(HttpContext context)
    {
        ...
        await WebAppBridgeResults.Json(
            context,
            new GeofenceStatusEvent(region.Identifier, state),
            LocationBridgeJsonContext.Default.GeofenceStatusEvent
        );
    }
}
...
public class WebAppGeofenceDelegate(WebAppEventHub events, WebAppInvoker invoker) : IGeofenceDelegate
{
    public virtual async Task OnStatusChanged(GeofenceState newStatus, GeofenceRegion region)
    {
        ...
        await invoker.InvokeAsync(
            "geofence",
            new GeofenceStatusEvent(region.Identifier, newStatus),
            LocationBridgeJsonContext.Default.GeofenceStatusEvent
        );
    }

    public static void Publish(WebAppEventHub events, GeofenceState state, GeofenceRegion region)
        => events.Publish("geofence.status", new GeofenceStatusEvent(region.Identifier, state), LocationBridgeJsonContext.Default.GeofenceStatusEvent);
}
...
public class WebAppGpsDelegate(WebAppInvoker invoker) : IGpsDelegate
{
    public virtual Task OnReading(GpsReading reading)
        => invoker.InvokeAsync("gps", GpsReadingResponse.From(reading), LocationBridgeJsonContext.Default.GpsReadingResponse);
}

public sealed record AccessResponse(AccessState Access);

public sealed record GpsListenerRequest(
    GpsBackgroundMode BackgroundMode = GpsBackgroundMode.None,
    bool RequestPreciseAccuracy = false,
    bool AutoRestart = true
)
{
    internal GpsRequest ToRequest() => new(this.BackgroundMode, this.RequestPreciseAccuracy, this.AutoRestart);

    internal static GpsListenerRequest From(GpsRequest request) => new(request.BackgroundMode, request.RequestPreciseAccuracy, request.AutoRestart);
}

public sealed record GpsReadingResponse(
    double Latitude,
    double Longitude,
...
{
    internal static GpsReadingResponse From(GpsReading r) => new(
        r.Position.Latitude,
...
public sealed record GeofenceRegionRequest(
    string Identifier,
    double Latitude,
    double Longitude,
    double RadiusMeters,
...
{
    internal bool TryToRegion(out GeofenceRegion? region, out string? error)
    {
        region = null;
        error = null;
        ...
        else if (this.Latitude is < -90 or > 90 || this.Longitude is < -180 or > 180)
            error = "latitude or longitude is out of range.";
        else if (this.RadiusMeters <= 0)
        ...
        region = new GeofenceRegion(
            this.Identifier,
            new Position(this.Latitude, this.Longitude),
    ...
    internal static GeofenceRegionRequest From(GeofenceRegion r) => new(
        r.Identifier,
        r.Center.Latitude,
        r.Center.Longitude,
...
public sealed record GeofenceStatusEvent(string Identifier, GeofenceState State);
...
[JsonSerializable(typeof(GpsListenerRequest))]
[JsonSerializable(typeof(GpsReadingResponse))]
[JsonSerializable(typeof(GeofenceRegionRequest))]
[JsonSerializable(typeof(List<GeofenceRegionRequest>))]
[JsonSerializable(typeof(GeofenceStatusEvent))]
partial class LocationBridgeJsonContext : JsonSerializerContext;
