...
public sealed class WebAppSettingsBridge : IWebAppBridge
{
    ...
    ValueTask ListAsync(HttpContext context)
    {
        ...
        return WebAppBridgeResults.Json(
            context,
            new WebAppSettingsList(scope, scope == "secure" && IsSecureStoreEncrypted, keys),
            WebAppStoreJsonContext.Default.WebAppSettingsList
        );
    }
    ...
    bool TryGetScope(HttpContext context, out string scope, out IKeyValueStore store)
    {
        scope = context.Request.RouteValues["scope"] ?? String.Empty;
...
public sealed record WebAppSettingsList(string Scope, bool Encrypted, IReadOnlyList<string> Keys);
...
[JsonSerializable(typeof(WebAppSettingsList))]
