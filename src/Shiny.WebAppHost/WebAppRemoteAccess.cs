...
public sealed class WebAppRemoteAccessOptions
{
    ...
    /// <summary>The bridge segment of <c>/_bridge/{name}/…</c>, or null when the path is not a bridge call.</summary>
    internal static string? BridgeName(string path)
    {
        if (!path.StartsWith(WebAppSession.BridgePrefix + "/", StringComparison.Ordinal))
            return null;

        var rest = path.AsSpan(WebAppSession.BridgePrefix.Length + 1);
