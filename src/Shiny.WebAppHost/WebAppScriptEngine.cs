...
/// </code>
/// <para>
/// What the script gets is deliberately small: <c>webapphost.on</c>, <c>console</c>, and <c>fetch</c> with
...
sealed class WebAppScriptEngine
{
    ...
    WebAppInvocationResult Run(LoadedScript script, string handler, string payloadJson, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var function = engine.Invoke("__webapphostHandler", handler);
    ...
    Engine CreateEngine(CancellationToken cancellationToken, Func<string, bool> register, Func<string, string, string, string?, string> fetch)
    {
        ...
        engine.SetValue("__webapphostRegister", new ClrFunction(engine, "__webapphostRegister", (_, args) =>
        {
            register(Arg(args, 0) ?? String.Empty);
            return JsValue.Undefined;
        }));

        engine.SetValue("__webapphostLog", new ClrFunction(engine, "__webapphostLog", (_, args) =>
        {
            this.Log(Arg(args, 0) ?? "info", Arg(args, 1) ?? String.Empty);
            return JsValue.Undefined;
        }));

        engine.SetValue("__webapphostFetch", new ClrFunction(engine, "__webapphostFetch", (_, args) =>
            JsString.Create(fetch(Arg(args, 0) ?? String.Empty, Arg(args, 1) ?? "GET", Arg(args, 2) ?? "{}", Arg(args, 3)))
        ));

        engine.Execute(HostScript, "webapphost-host.js");
    ...
    const string HostScript = """
        (function (global) {
            ...
            global.webapphost = Object.freeze({
                on(name, handler) {
                    if (typeof name !== "string" || typeof handler !== "function")
                        throw new TypeError("webapphost.on(name, handler) takes a string and a function");

                    handlers[name] = handler;
                    __webapphostRegister(name);
                }
            });

            global.__webapphostHandler = name => handlers[name];

            const write = level => (...args) => __webapphostLog(level, args.map(a => typeof a === "string" ? a : JSON.stringify(a)).join(" "));
            ...
            global.fetch = (input, init) => {
                try {
                    ...
                    const raw = JSON.parse(__webapphostFetch(
                        String(input),
