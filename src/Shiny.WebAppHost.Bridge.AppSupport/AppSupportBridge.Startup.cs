...
public sealed partial class AppSupportBridge
{
    ...
    ValueTask StartupStateAsync(HttpContext context)
        => this.startup is { IsSupported: true } s
            ...
            : WebAppBridgeResults.Json(
                context,
                new StartupStateResponse(false, StartupServiceState.NotSupported),
                StartupBridgeJsonContext.Default.StartupStateResponse
            );
    ...
    async ValueTask OpenStartupSettingsAsync(HttpContext context)
    {
        ...
        await WebAppBridgeResults.Json(context, new StartupSettingsResponse(opened), StartupBridgeJsonContext.Default.StartupSettingsResponse);
    }
    ...
    static async ValueTask StartupResult(HttpContext context, Func<Task<StartupServiceState>> action, bool supported)
    {
        try
        {
            await WebAppBridgeResults.Json(
                context,
                new StartupStateResponse(supported, await action()),
                StartupBridgeJsonContext.Default.StartupStateResponse
            );
        }
...
public sealed record StartupStateResponse(bool Supported, StartupServiceState State);

public sealed record StartupSettingsResponse(bool Opened);
...
[JsonSerializable(typeof(StartupStateResponse))]
[JsonSerializable(typeof(StartupSettingsResponse))]
partial class StartupBridgeJsonContext : JsonSerializerContext;
