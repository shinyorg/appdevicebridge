...
namespace Shiny.WebAppHost.Bridge.Push;
...
public sealed class PushBridge(IServiceProvider services) : IWebAppBridge
{
    ...
    async ValueTask StatusAsync(HttpContext context)
    {
        ...
        await WebAppBridgeResults.Json(
            context,
            new PushStatusResponse(access, p.RegistrationToken, p.NativeRegistrationToken),
            PushBridgeJsonContext.Default.PushStatusResponse
        );
    }
    ...
    async ValueTask RegisterAsync(HttpContext context)
    {
        ...
        await WebAppBridgeResults.Json(
            context,
            new PushStatusResponse(state.Status, state.RegistrationToken ?? p.RegistrationToken, p.NativeRegistrationToken),
            PushBridgeJsonContext.Default.PushStatusResponse
        );
    }
    ...
    ValueTask GetTagsAsync(HttpContext context)
    {
        ...
        return WebAppBridgeResults.Json(context, new PushTags(p.TryGetTags() ?? []), PushBridgeJsonContext.Default.PushTags);
    }
    ...
    async ValueTask SetTagsAsync(HttpContext context)
    {
        ...
        if (await WebAppBridgeResults.ReadBodyAsync(context, PushBridgeJsonContext.Default.PushTags) is not { } body)
        {
...
public class WebAppPushDelegate(WebAppEventHub events, WebAppInvoker invoker, WebAppPushOptions options) : IPushDelegate
{
    ...
    {
        events.Publish("push.token", new PushTokenEvent(token), PushBridgeJsonContext.Default.PushTokenEvent);
        return Task.CompletedTask;
    }

    public virtual Task OnUnRegistered(string token)
    {
        events.Publish("push.unregistered", new PushTokenEvent(token), PushBridgeJsonContext.Default.PushTokenEvent);
        return Task.CompletedTask;
    }
    ...
    {
        var payload = PushPayload.From(notification);
        ...
        events.Publish(name, payload, PushBridgeJsonContext.Default.PushPayload);
        await invoker.InvokeAsync(name, payload, PushBridgeJsonContext.Default.PushPayload);
    }
}

public sealed record PushStatusResponse(AccessState Access, string? Token, string? NativeToken);
public sealed record PushTags(string[] Tags);
public sealed record PushTokenEvent(string Token);

public sealed record PushPayload(Dictionary<string, string> Data, string? Title, string? Message)
{
    internal static PushPayload From(PushNotification notification) => new(
