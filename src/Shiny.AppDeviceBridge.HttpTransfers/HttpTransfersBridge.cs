...
using Contracts = Shiny.AppDeviceBridge.HttpTransfers.Client;

namespace Shiny.AppDeviceBridge.HttpTransfers;
...
public sealed class HttpTransfersBridge : IWebAppBridge, IDisposable
{
    ...
    async ValueTask ListAsync(HttpContext context)
    {
        ...
        await WebAppBridgeResults.Json(
            context,
            new Contracts.TransferList([.. transfers.Select(x => this.scope.Describe(x.Id, x.Transfer))]),
            Contracts.TransfersJsonContext.Default.TransferList
        );
    }
    ...
    async ValueTask CreateAsync(HttpContext context)
    {
        ...
        var body = await WebAppBridgeResults.ReadBodyAsync(context, Contracts.TransfersJsonContext.Default.TransferRequest);
        ...
        {
            await (this.scope.HasRoot(body.Root)
                // A picked folder on Android is a content URI; the transfer service can only write to a path.
                ? WebAppBridgeResults.BadRequest(context, $"File root '{body.Root}' has no path on disk, which transfers need.")
                : WebAppBridgeResults.NotFound(context, $"No file root '{body.Root}'."));
            return;
        }
        ...
        var type = BridgeEnum.Convert<Contracts.TransferType, TransferType>(body.Type);
    ...
    ValueTask GetAsync(HttpContext context, IHttpTransferManager m, string id, HttpTransfer transfer)
        => WebAppBridgeResults.Json(context, this.scope.Describe(id, transfer), Contracts.TransfersJsonContext.Default.TransferInfo);
    ...
    static string? ValidateRequest(Contracts.TransferRequest body, TransferType type)
    {
...
) : IHttpTransferDelegate
{
    ...
    readonly WebAppTransferScope scope = new(hostOptions, roots);
...
sealed class WebAppTransferScope
{
    ...
    // Read each time: a root can be added while the app runs. Only roots on disk: the transfer service writes to a path.
    IEnumerable<WebAppFileRoot> Roots => this.fileRoots.All.OfType<WebAppFileRoot>();
    ...
    public bool HasRoot(string? name) => this.fileRoots.TryGet(name, out _);

    public WebAppFileRoot? FindRoot(string? name)
        => this.fileRoots.TryGet(name, out var root) ? root as WebAppFileRoot : null;
    ...
    public Contracts.TransferInfo Describe(string id, HttpTransfer transfer)
    ...
    public Contracts.TransferInfo Describe(string id, HttpTransferRequest request, HttpTransferState status, TransferProgress progress, DateTimeOffset? createdAt = null)
    {
        ...
        return new Contracts.TransferInfo(
            id,
            BridgeEnum.Convert<TransferType, Contracts.TransferType>(request.Type),
            request.Uri,
            file?.Root.Name,
            file?.Path,
            BridgeEnum.Convert<HttpTransferState, Contracts.TransferState>(status),
