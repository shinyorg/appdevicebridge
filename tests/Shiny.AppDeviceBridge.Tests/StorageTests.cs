...
public class WebAppFileRootTests
{
    ...
    {
        var root = new WebAppFileRoot("data", Path.Combine(Path.GetTempPath(), "appdevicebridge-tests", "root"));
...
public class WebAppFileRootsTests
{
    ...
    public void ResolvesAFileInANamedRoot()
    {
        ...
        Assert.True(new WebAppFileRoots(options).TryResolve("DATA", "photos/cat.jpg", out var full));
    ...
    public void RefusesWhatTheFilesBridgeWould(string? root, string path)
        => Assert.False(new WebAppFileRoots(Options()).TryResolve(root, path, out _));

    [Fact]
    public void RefusesEverythingWhenFilesAreDisabled()
        => Assert.False(new WebAppFileRoots(Options(enableFiles: false)).TryResolve("data", "a.txt", out _));
}
...
public class StorageBridgeTests
{
    ...
    public async Task SettingsRoundTripAndStayInTheirLane()
    {
        ...
        var listing = await webView.GetFromJsonAsync("/_bridge/settings/local", AppDeviceBridgeJsonContext.Default.SettingsList);
        ...
        Assert.Empty((await webView.GetFromJsonAsync("/_bridge/settings/local", AppDeviceBridgeJsonContext.Default.SettingsList))!.Keys);
    ...
    public async Task FilesCoverTheLifecycle()
    {
        ...
        var root = await webView.GetFromJsonAsync<List<FileEntry>>("/_bridge/files/data/list");
        Assert.NotNull(root);
        Assert.Equal(["archive", "notes"], root.Select(x => x.Name));
        Assert.All(root, x => Assert.True(x.IsDirectory));

        var info = await webView.GetFromJsonAsync<FileEntry>("/_bridge/files/data/info?path=notes/today.txt");
        Assert.Equal(new FileEntry("today.txt", "notes/today.txt", false, 12, info!.Modified), info);

        // rename, move, copy
        var renamed = await webView.PostAsJsonAsync("/_bridge/files/data/move", new FileTransfer("notes/today.txt", "notes/yesterday.txt"));
        Assert.Equal(HttpStatusCode.OK, renamed.StatusCode);

        var moved = await webView.PostAsJsonAsync("/_bridge/files/data/move", new FileTransfer("notes", "archive/2026/notes"));
        ...
        var copied = await webView.PostAsJsonAsync("/_bridge/files/data/copy", new FileTransfer("archive", "backup"));
        ...
        Assert.Equal(HttpStatusCode.Conflict, (await webView.PostAsJsonAsync("/_bridge/files/data/copy", new FileTransfer("archive", "backup"))).StatusCode);
        Assert.Equal(HttpStatusCode.BadRequest, (await webView.PostAsJsonAsync("/_bridge/files/data/move", new FileTransfer("archive", "archive/inner"))).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.BadRequest, (await webView.PostAsJsonAsync("/_bridge/files/data/copy", new FileTransfer("archive", "../stolen"))).StatusCode);
    ...
    public async Task FilesRefuseWritesOverTheLimit()
    {
        ...
        var listing = await webView.GetFromJsonAsync<List<FileEntry>>("/_bridge/files/data/list");
