...
public class BridgeSecurityTests
{
    ...
    public async Task LetsTheAppDecideWhoMayCall()
    {
        await using var fixture = await RemoteFixture.StartAsync(o =>
        {
            ...
            o.AuthorizeBridges(p => p.RequireRole("kiosk"));
        });
    ...
    public async Task RefusesAForeignOriginOnTheDevice()
    {
        ...
        await using var server = BridgesOnly(app.BridgeOptions());
    ...
    [Fact]
    public async Task TakesEveryServerSettingAndHook()
    {
        await using var app = new TestApp();
        var hooked = 0;

        await using var server = BridgesOnly(app.BridgeOptions(o =>
        {
            o.Server.Limits.MaxRequestHeaderCount = 50;
            o.ConfigureServer((http, _) =>
            {
                ...
                http.MapGet("/api/open", ctx => ctx.Response.WriteAsync("open")).AllowAnonymous();
            });
        }));
        var origin = await server.StartAsync();

        using var client = new HttpClient();
        var response = await client.GetAsync(new Uri(origin, "/api/open"));
    ...
    static AppDeviceBridgeServer BridgesOnly(AppDeviceBridgeOptions options)
        => new(options, [new EchoBridge()], new WebAppEventHub());
    ...
    sealed class RemoteFixture : IAsyncDisposable
    {
        ...
        public static async Task<RemoteFixture> StartAsync(Action<AppDeviceBridgeOptions>? configure = null, Action<WebAppHostOptions>? web = null)
        {
...
public class BridgeCallerTests
{
    ...
    public void DefaultsToAReleaseBuildsRulesOutsideTheDebugger()
    {
        ...
        Assert.Equal(IPAddress.Loopback, options.Server.Address);
        Assert.Equal(5780, options.Server.Port);
    }
}

/// <summary>The container wiring an app gets from AddAppDeviceBridge and AddWebAppHost.</summary>
public class RegistrationTests
{
    ...
    public async Task WiresTheWebViewHostOntoTheServer()
    {
        ...
        services.AddAppDeviceBridge(o => o.AppId = TestApp.AppId);
        ...
        // Every call configures the same options.
    ...
    public void ValidatesWhenTheServerIsCreated()
    {
        ...
        var failure = Assert.Throws<InvalidOperationException>(() => provider.GetRequiredService<AppDeviceBridgeServer>());
