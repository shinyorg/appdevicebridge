...
public class RemoteAccessOptionTests
{
    ...
    public void ReadsTheBridgeNameOutOfThePath(string path, string? expected)
        => Assert.Equal(expected, WebAppRemoteAccessOptions.BridgeName(path, "/_bridge"));

    [Theory]
    [InlineData("/kiosk/_native/files/data/list", "files")]
    [InlineData("/kiosk/_native/host", "host")]
    [InlineData("/_bridge/files", null)]            // the default prefix, on a host that moved it
    [InlineData("/kiosk/_native", null)]
    public void ReadsItRelativeToWhereverTheBridgesWereMounted(string path, string? expected)
        => Assert.Equal(expected, WebAppRemoteAccessOptions.BridgeName(path, "/kiosk/_native"));
    ...
    public void RefusesAnAllowlistThatCannotTakeEffect()
    {
        ...
        options.UseBaseline(typeof(RemoteAccessOptionTests).Assembly, "Shiny.AppDeviceBridge.Tests.baseline.zip");
