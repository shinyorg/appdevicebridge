#!/usr/bin/env bash
# Drives the Shiny.WebAppHost sample on macOS: the release server, `dotnet watch` on Sample.Blazor, and the
# AppKit head (samples/Sample.MacOS). Run from the repository root:
#
#   .claude/skills/run-webapphost/driver.sh <command>
#
# Commands:
...
#   shot [name]       bring the app forward and screenshot its window -> artifacts/run-webapphost/<name>.png
...
OUT="$ROOT/artifacts/run-webapphost"
APP="$ROOT/samples/Sample.MacOS/bin/Debug/net10.0-macos/osx-arm64/WebAppHost Sample.app"
...
for w in windows where (w[kCGWindowOwnerName as String] as? String) == "WebAppHost Sample" && (w[kCGWindowLayer as String] as? Int) == 0 {
...
cmd_test() { dotnet test "$ROOT/tests/Shiny.WebAppHost.Tests/Shiny.WebAppHost.Tests.csproj" -nologo; }
