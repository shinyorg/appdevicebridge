...
`.claude/skills/run-webapphost/driver.sh` starts them in the right order, screenshots the app window, clicks in it,
and triggers hot reload. Screenshots land in `artifacts/run-webapphost/`.
...
```bash
.claude/skills/run-webapphost/driver.sh up
.claude/skills/run-webapphost/driver.sh status
```
...
```bash
perl -e 'select(undef,undef,undef,20)' && .claude/skills/run-webapphost/driver.sh shot home
```
...
```bash
.claude/skills/run-webapphost/driver.sh click 459 55     # the "Device" nav link
.claude/skills/run-webapphost/driver.sh shot device
```

Hot reload changes the Home page `<h1>` in place and waits for `dotnet watch` to report it:

```bash
.claude/skills/run-webapphost/driver.sh heading "WebAppHost + Blazor (hot reloaded)"
.claude/skills/run-webapphost/driver.sh heading "WebAppHost + Blazor"
```

The rest:

```bash
.claude/skills/run-webapphost/driver.sh logs     # dotnet watch output
.claude/skills/run-webapphost/driver.sh test     # 73 unit and integration tests
.claude/skills/run-webapphost/driver.sh down
```
